...
// The table class for custom post types
class Register_Post_Types_and_Taxonomies__Post_Types extends WP_List_Table {
	...
	function column_cb( $item ) {
		...
		if ( ! empty( $item['label'] ) ) {
			...
			$html = '<label class="screen-reader-text" for="cb-select-' . $id . '">' . sprintf( __( 'Select %s', 'register_post_types_and_taxonomies' ), $label ) . '</label>'
					. '<input type="checkbox" name="select_post_types[]" value="' . $id . '" id="cb-select-' . $id . '" />';
